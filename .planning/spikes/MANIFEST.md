...
| 011 | offline-agent-local-routing | offline-agent-local-routing | standard | Given a sub-1B local model, when asked for today's sales in English or Spanish, then it can emit only the allow-listed read-only request and the application rejects invented tools/arguments | PARTIAL | local-ai, offline, security, model-routing |
