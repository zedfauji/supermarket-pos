...
`local-routing-spike.cjs` uses only a loopback Ollama endpoint for this machine's experiment. It is not production architecture and it makes no database call: `reports.today_sales` returns a fixed fixture. That is deliberate because the current POS has no local authoritative replica when Supabase is unreachable. It explicitly refuses yesterday's report before inference rather than silently returning today's fixture, and it formats an approved result locally so model output cannot echo an injected request.
