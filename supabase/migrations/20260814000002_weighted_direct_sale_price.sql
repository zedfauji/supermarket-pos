-- Correct weighted direct-sale pricing: persisted unit_price is the rounded line
-- total because order_items.quantity remains the integer sentinel 1. The RPC
-- derives it from the catalog's per-kg base_price and supplied grams.
...
BEGIN
  SELECT id, tab_id, payment_group_id INTO v_existing_payment_id, v_existing_tab_id, v_existing_group_id
  FROM payments WHERE idempotency_key IN (p_idempotency_key, p_idempotency_key || '-leg0') ORDER BY processed_at LIMIT 1;
  IF FOUND THEN
  ...
  FOR v_elem IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    SELECT base_price, sold_by_weight INTO v_catalog_price, v_sold_by_weight FROM products WHERE id = (v_elem->>'product_id')::uuid;
  ...
  SELECT v_order_id, (elem->>'product_id')::uuid, COALESCE((elem->>'quantity')::int, 1), (elem->>'unit_price')::numeric,
    ...
    COALESCE((elem->>'modifier_price_delta')::numeric, 0), NULLIF(elem->>'notes', ''), NULLIF((elem->>'weight_grams')::text, '')::integer
  ...
  IF p_legs IS NULL THEN
    v_result := process_payment_atomic(p_tab_id := v_tab_id, p_staff_id := p_staff_id, p_amount := p_amount, p_tip_amount := p_tip_amount,
      ...
      p_discount_value := p_discount_value, p_discount_amount := p_discount_amount);
  ELSE
    v_result := process_split_payment_atomic(p_tab_id := v_tab_id, p_staff_id := p_staff_id, p_legs := p_legs,
      ...
      p_discount_type := p_discount_type, p_discount_value := p_discount_value, p_discount_amount := p_discount_amount);
  END IF;
