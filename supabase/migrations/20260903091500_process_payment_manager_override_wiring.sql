-- =============================================================================
-- Phase 27 Plan 09 gap closure (G-27-13, secondary finding): unlike
-- process_direct_sale_atomic (fixed in Plan 08,
-- 20260903090000_process_direct_sale_manager_pin_reverify.sql),
-- process_payment_atomic/process_split_payment_atomic — the RPCs behind the
-- reopened-tab (PaymentPane) payment screen — had NO manager-override
-- authorization check at all: a discount request with p_discount_amount set
-- was accepted with zero verification of who authorized it (T-27-12).
--
-- Root cause: .planning/debug/adhoc-discount-manager-pin-rejected-after-entry.md
-- (secondary finding, same debug session as Plan 08's primary fix).
--
-- Fix: give process_payment_atomic and process_split_payment_atomic the SAME
-- PIN-based re-verification pattern Plan 08 gave process_direct_sale_atomic —
-- append p_manager_override boolean DEFAULT false, p_manager_pin text
-- DEFAULT NULL::text as the LAST two parameters (Pitfall 5 from
-- 27-01-SUMMARY.md: appended only, never inserted mid-list), and independently
-- re-derive the authorizing staff from profiles.pin = p_manager_pin before
-- accepting any discount field. Then re-CREATE process_direct_sale_atomic so
-- its own internal delegation to these two functions forwards its
-- already-validated p_manager_override/p_manager_pin through — otherwise the
-- new inner check would reject every ad-hoc-discounted direct sale a second
-- time, this time with the delegation call's default false/NULL values
-- instead of the real PIN already verified at process_direct_sale_atomic's
-- own top (T-27-13, defense-in-depth, not a new trust hole: the forwarded
-- values are process_direct_sale_atomic's OWN already-validated values, never
-- re-derived from an untrusted source at the delegation boundary).
--
-- Everything else in all three functions — the "fully covered" discount-aware
-- close check, the bank_transfer GUC check, the per-leg loop, the promotion/
-- floor-guard logic — is copied verbatim from the current live definitions
-- (20260902000001_close_tab_accounts_for_adhoc_discount.sql for the first two,
-- 20260903090000_process_direct_sale_manager_pin_reverify.sql for the third),
-- unchanged. This migration is scoped to wiring manager-override authorization
-- through, nothing else.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Deviation (Rule 1/3 auto-fix, discovered applying this migration locally):
-- Postgres identifies a function by name + ORDERED INPUT TYPE LIST, not by
-- name alone — CREATE OR REPLACE FUNCTION can only replace a function whose
-- argument types are IDENTICAL to an existing one. Appending a new trailing
-- parameter (even with a DEFAULT) changes that type list, so CREATE OR
-- REPLACE silently creates a SECOND overload instead of truly replacing the
-- old one. The pre-this-migration signature is left permanently reachable —
-- and, worse, a named-argument RPC call that omits the new optional
-- parameters becomes AMBIGUOUS between both overloads:
--   ERROR: function process_direct_sale_atomic(...) is not unique
--   HINT: Could not choose a best candidate function.
-- This was reproduced locally (psql) for all three functions this migration
-- touches, including process_direct_sale_atomic (Plan 08 already hit the same
-- root cause when it appended p_manager_pin without dropping the prior
-- 17-param signature — this migration also fixes that pre-existing leftover
-- overload as a byproduct of touching the same function again). Explicit
-- DROP FUNCTION IF EXISTS on each function's pre-this-migration signature,
-- immediately before its CREATE OR REPLACE, is the standard fix for
-- Postgres's function-identity semantics when a parameter list grows.
-- -----------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.process_payment_atomic(uuid, uuid, numeric, text, text, numeric, text, text, text, text, numeric, numeric, integer, text);
DROP FUNCTION IF EXISTS public.process_split_payment_atomic(uuid, uuid, jsonb, numeric, text, text, text, numeric, numeric, integer);
DROP FUNCTION IF EXISTS public.process_direct_sale_atomic(uuid, uuid, uuid, jsonb, text, text, numeric, numeric, text, jsonb, numeric, text, text, numeric, numeric, text, text, boolean);
...
-- -----------------------------------------------------------------------------
-- 2. process_split_payment_atomic
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 3. process_direct_sale_atomic — re-CREATE OR REPLACE starting from Plan 08's
--    live body (20260903090000_process_direct_sale_manager_pin_reverify.sql),
--    changing ONLY the two internal delegation calls near the bottom: forward
--    p_manager_override/p_manager_pin so its own already-approved direct
--    sales are not newly rejected by the inner checks added above (steps 1/2).
-- -----------------------------------------------------------------------------
