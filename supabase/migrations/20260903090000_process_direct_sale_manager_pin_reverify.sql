-- =============================================================================
-- Phase 27 Plan 08 gap closure (G-27-13, blocker): process_direct_sale_atomic's
-- manager-override re-check was keyed on the CALLER's own identity
-- (`p.id = p_staff_id`, resolved from the caller's own JWT), never the staff
-- whose PIN was actually entered into ManagerPinDialog. A cashier who gets a
-- real manager to type their PIN into the dialog still failed this check with
-- FORBIDDEN, because the cashier's own role_permissions row (not
-- apply_custom_discount-eligible) was what got re-checked — the manager's PIN
-- entry had no effect on the server-side decision at all.
--
-- Root cause: .planning/debug/adhoc-discount-manager-pin-rejected-after-entry.md
--
-- Fix: append p_manager_pin text DEFAULT NULL::text as the LAST parameter
-- (Pitfall 5 from 27-01-SUMMARY.md — appended only, never inserted mid-list,
-- or PostgREST's schema cache / Postgres's function-identity resolution
-- breaks) and re-key the manager-override lookup on `p.pin = p_manager_pin`
-- instead of `p.id = p_staff_id`. The server now independently re-derives the
-- authorizing staff from the raw PIN string itself — mirroring exactly the
-- lookup the client already performs client-side (ManagerPinDialog's
-- `eligibleStaff.find(s => s.pin === enteredPin)`), except this version can't
-- be tampered with by a modified client, since it never trusts a
-- client-supplied staff id (see 27-08-PLAN.md's threat_model, T-27-10).
--
-- Everything else — the promotion/floor-guard per-item loop, the ad-hoc
-- discount computation, the DISCOUNT_REQUIRES_MANAGER/INVALID_DISCOUNT_SCOPE/
-- INVALID_DISCOUNT_PARAMS checks — is copied verbatim from
-- 20260902000002_process_direct_sale_atomic_discount_param_validation.sql
-- (the current live definition), unchanged. This migration is scoped to the
-- identity source of ONE check.
--
-- Note: profiles.pin has no UNIQUE constraint (only a partial index,
-- idx_profiles_pin) — if two active staff happen to share a PIN, this lookup
-- can match either. This is a pre-existing characteristic of the app's PIN
-- model (the client's own eligibleStaff.find has the identical ambiguity
-- already) and is not a new regression introduced here.
-- =============================================================================
