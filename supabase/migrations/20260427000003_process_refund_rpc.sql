-- =============================================================================
-- S4-07: process_refund RPC
--
-- Processes a refund against an original payment. All operations are atomic.
-- SECURITY DEFINER: runs as function owner; auth.uid() preserved by Supabase JWT.
--
-- Security controls (T-06-04 through T-06-08):
--   - Manager/admin role check via auth.uid() join on profiles
--   - REFUND_EXCEEDS_ORIGINAL guard: sum(existing refunds) + new refund ≤ original amount
--   - ITEM_NOT_IN_ORIGINAL_ORDER: verifies each item belongs to the payment's tab
--   - Negative payment row inserted with is_refund=true (valid via amount_positive CHECK)
--   - deplete_for_order_item stub: graceful fallback if Phase 4 function not yet deployed
--   - audit_log stub: graceful fallback if audit_log table not yet created
--
...
-- DOWN:
-- BEGIN;
-- REVOKE EXECUTE ON FUNCTION process_refund(uuid, jsonb, text, text) FROM authenticated;
-- DROP FUNCTION IF EXISTS process_refund(uuid, jsonb, text, text);
-- COMMIT;
-- =============================================================================
