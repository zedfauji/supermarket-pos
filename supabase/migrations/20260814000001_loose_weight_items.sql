-- Phase 02 decision: gram-accurate loose-weight checkout.
-- For products.sold_by_weight = true, inventory.quantity_on_hand is stored in GRAMS.
-- Regular products continue to store whole saleable units. Do not aggregate or compare
-- inventory.quantity_on_hand across product types without checking sold_by_weight.

ALTER TABLE order_items ADD COLUMN weight_grams INTEGER NULL;
ALTER TABLE order_items
  ADD CONSTRAINT weight_grams_range
...
COMMENT ON COLUMN inventory.quantity_on_hand IS
  'Units on hand for regular products; GRAMS on hand for products.sold_by_weight = true products (Phase 2 decision, see 20260814000001 migration comment) — never sum/compare across product types without checking sold_by_weight first.';
...
BEGIN
  ...
  INSERT INTO stock_movements (product_id, quantity_delta, reason, staff_id)
  SELECT OLD.product_id, v_restore, 'correction', o.staff_id
  FROM orders o
...
DECLARE
  ...
  v_result jsonb;
BEGIN
  SELECT id, tab_id, payment_group_id
  INTO v_existing_payment_id, v_existing_tab_id, v_existing_group_id
  FROM payments
  WHERE idempotency_key IN (p_idempotency_key, p_idempotency_key || '-leg0')
  ORDER BY processed_at
  LIMIT 1;

  IF FOUND THEN
    IF v_existing_group_id IS NOT NULL THEN
      SELECT array_agg(id ORDER BY split_index)
      INTO v_existing_payment_ids
      FROM payments
      WHERE payment_group_id = v_existing_group_id;
      RETURN jsonb_build_object(
        'ok', true,
        'idempotent', true,
        'tabId', v_existing_tab_id,
        'paymentGroupId', v_existing_group_id,
        'paymentIds', to_jsonb(v_existing_payment_ids)
      );
    END IF;
    RETURN jsonb_build_object(
      'ok', true,
  ...
  LOOP
    ...
    IF (v_elem->>'weight_grams') IS NOT NULL
      AND ((v_elem->>'weight_grams')::int <= 0 OR (v_elem->>'weight_grams')::int > 50000) THEN
  ...
  INSERT INTO order_items (
    order_id, product_id, quantity, unit_price, modifier_ids, modifier_price_delta, notes, weight_grams
  )
  SELECT
    v_order_id,
    ...
    NULLIF(elem->>'notes', ''),
    NULLIF((elem->>'weight_grams')::text, '')::integer
  ...
  RETURN jsonb_build_object(
    ...
    'paymentGroupId', v_result->>'paymentGroupId',
