...
--
-- Atomically inserts a parent order_item (is_combo=true) + N child order_items
-- (one per slot selection, price=0) for a given combo product and tab.
-- Pool-time slots are tracked via pool_sessions.prepaid_minutes; the actual
-- session start is handled by the start-pool-timer feature (which already
-- applies prepaid_minutes first — see plan 02-02 pool-billing extension).
-- =============================================================================
