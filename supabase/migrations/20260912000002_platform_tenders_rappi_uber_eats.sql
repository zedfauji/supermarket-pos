-- Configurable payment methods: rappi and uber_eats become "platform
-- tenders" — the delivery platform already collected the money, so at the
-- POS they behave exactly like 'card' (no tendered amount, optional
-- free-text reference carrying the platform order id in reference_number,
-- tax applies normally, discounts allowed, no cash drawer). The bar-pos-era
-- Rappi coupling to a tab-level rappi_order_id (RAPPI_ORDER_MISMATCH guard)
-- is removed for both platform tenders, because the rappi_orders table (and
-- the whole bar/pool-parlour delivery-order concept it backed) was dropped
-- end-to-end in Phase 1 (2026-08-10) — tabs.rappi_order_id is a vestigial
-- column with nothing left populating it meaningfully for this use case.
--
-- Functions touched (re-CREATE OR REPLACE, same signature as the latest live
-- definition — no DROP FUNCTION needed since no parameter is added/removed):
--   1. process_payment_atomic         — allow-list + drop RAPPI_ORDER_MISMATCH
--   2. process_split_payment_atomic   — per-leg allow-list + drop per-leg check
--   3. process_direct_sale_atomic     — method/legs guard allow-list only
...
-- -----------------------------------------------------------------------------
-- 4. get_caja_report — add v_uber_eats_sales alongside v_rappi_sales; body
--    otherwise verbatim from 20260831000004_caja_report_bank_transfer_breakout.sql.
-- -----------------------------------------------------------------------------
